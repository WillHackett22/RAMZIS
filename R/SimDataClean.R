...
#' @param rel Pick if values should be normalized relative to the max total abundance of any column('Within'), to a given value (NUMERIC), or read as is('AsIs'). Recommended numeric is the max of both columns for a given value. Default = 'Within'
...
SimDataClean<-function(filename,kmin=2,rel='Within',normvector='None',logoption=TRUE){
