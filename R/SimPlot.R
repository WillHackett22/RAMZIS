...
#' @param SimilarityObj Test and Null Similarity Object from SymmetricalSimBootstrap()
...
SimPlot<-function(PlotTitle,SimilarityObj,legendpos='topleft',verbose=F,legendval=FALSE,xbound=c(0,1),ybound=T,logval=FALSE,legendcexval=1.1){
  ...
  if (max(TDis)==0){
    ...
    graphics::polygon(c(0,0,0.05,0.05),c(0,mh,mh,0),col=grDevices::rgb(1,0,0,0.5))
  } else {
    mh<-max(c(sdNy,sdTy))
    if (ybound==T){
      yubound<-c(0,mh)
    ...
    graphics::polygon(c(-0.1001,dT$x,1.1001),c(0,sdTy,0),col=grDevices::rgb(1,0,0,0.5))
  }
  graphics::lines(dN$x,sdNy)
  graphics::polygon(c(-0.1001,dN$x,1.1001),c(0,sdNy,0),col=grDevices::rgb(0,0,1,0.5))
  graphics::lines(rep(TAct,2),c(0,mh),col=1,lwd=3)
  ...
  if (legendval){
    graphics::legend(legendpos,cex=legendcexval,legend=c(paste('Observed Similarity of ',round(TAct,2),' in the',round(ecdf(TDis)(TAct),2)*100,'Percentile'),'Test Distribution','Null Distribution',paste0(round(AlphaValue,3)*100,'% False Positive Rate'),paste0(round(BetaValue,3)*100,'% False Negative Rate')),fill=c(NA,grDevices::rgb(1,0,0,0.5),grDevices::rgb(0,0,1,0.5),'black','darkgray'),lty=c(1,rep(NA,4)),density=c(0,NA,NA,NA,NA),border=c(NA,1,1,1,1))
  }
  ...
  if (AlphaValue>(10^-10)){
    ...
    for (j in 1:(length(OverlapData$FPi)/2)){
      ...
      graphics::polygon(c(xi,xi,dT$x[fpi],x0,x0),c(0,sdTy[fpi][1],sdTy[fpi],sdTy[fpi[length(fpi)]],0),col='black')
    }
  }
  if (BetaValue>(10^-10)){
    #make beta area
    for (j in 1:(length(OverlapData$FNi)/2)){
      ...
      graphics::polygon(c(xi,dN$x[fni],x0,x0),c(0,sdNy[fni],sdNy[fni[length(fni)]],0),col='darkgrey')
    }
  }
